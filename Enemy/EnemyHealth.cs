...
public class EnemyHealth : MonoBehaviour {
	...
	// We spawn two eyes when we die. For gore reasons
	...
	void SpawnPickup() {
		...
		if (rand <= 0.2f) {
            ...
            // Rapid.
                ...
                Instantiate(pickupManager.rapidPickup, spawnPosition, transform.rotation);
            }
			// Health.
			else {
        ...
        // Spawn a speed bost when we reach a certain score past wave 1.
        if (scoreManager.GetScore() == pickupManager.scoreNeededForSpeedBoost && SceneManager.GetActiveScene().buildIndex == 2) {
            Instantiate(pickupManager.speedPickup, spawnPosition, transform.rotation);
        }
        else if (scoreManager.GetScore() == 1200 && SceneManager.GetActiveScene().buildIndex == 3) {
            Instantiate(pickupManager.speedPickup, spawnPosition, transform.rotation);
        }
        ...
        // Spawn an demon pickup when we reach a certain score.
        /*else if (scoreManager.GetScore() == pickupManager.scoreNeededForDemon) {
            Instantiate(pickupManager.demonPickup, spawnPosition, transform.rotation);

        // Increase the score needed to spawn a demon pickup.
         pickupManager.scoreNeededForDemon += pickupManager.extraScoreNeededAfterEachDemon;
        }*/
    }
}
